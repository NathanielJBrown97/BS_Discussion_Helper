...
# Function: Determines if response contains adequate information on information collection.
...
def score_info_collection_content(response):
    info_collect_keywords = ["information should collect", "visitor"]
    algorithm_description_keywords = ["informal algorithm", "fields", "validate", "user input"]
    examples_evaluation_keywords = ["work for the visitors", "change your program", "fail"]
    falsehoods_reflection_keywords = ["Falsehoods Programmers Believe", "care", "approach differently"]

    score = 0

    if any(keyword in response.lower() for keyword in info_collect_keywords):
        score += 1
    if any(keyword in response.lower() for keyword in algorithm_description_keywords + examples_evaluation_keywords + falsehoods_reflection_keywords):
        score += 1
...
def main():
    ...
    file_path_to_csv = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'gather_responses', 'temp', 'live_week10.csv'))
    ...
    df['Content of Original Post'] = df['response'].apply(score_info_collection_content)
    ...
    output_file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'results', 'week10_csc201_graded.xlsx'))
